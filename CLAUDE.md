...
- [x] **Archivage local + purge serveur (2026-07-09)** : `ops/sync_supabase_local.py` — réplique TOUTES les tables Supabase dans `archive/lowi-archive.db` (SQLite, gitignoré, introspection de schéma → résiste aux évolutions), puis `--prune` supprime du serveur les annonces inactives délistées >90 j **uniquement si leur copie est vérifiée id par id dans l'archive** (garde-fous : archive < serveur → purge interdite ; candidates absentes → annulée). Le local = référence historique complète ; le serveur = fenêtre chaude. **Tâche Windows `LowiBKK-ArchiveSync`** (hebdo, dim. 21:00, wrapper `ops/sync-archive.ps1`, logs `ops/logs/`) — suppression : `schtasks /Delete /TN "LowiBKK-ArchiveSync" /F`.
